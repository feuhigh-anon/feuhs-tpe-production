...
- The current Supabase table groups and roster import lifecycle are documented in [docs/DATABASE_LIFECYCLE.md](docs/DATABASE_LIFECYCLE.md). Application tables remain domain-oriented; imported but inactive roster data uses the `roster_stage_` prefix.
- The synthetic technical pilot workflow, current-question testing boundary, and safe table-export commands are documented in [docs/PILOT_TEST_WORKFLOW.md](docs/PILOT_TEST_WORKFLOW.md).
- The evidence-supported item review and validity/reliability plan are documented in [docs/TPE_LITERATURE_REVIEW.md](docs/TPE_LITERATURE_REVIEW.md), alongside the initial item-level review in [docs/TPE_INSTRUMENT_REVIEW.md](docs/TPE_INSTRUMENT_REVIEW.md).
